...
画面の指示に従い、デバイスのカメラを使用してアプリ内で、名刺を撮影してください。<br>
撮影された画像から文字を読み取り、表示します。<br>
参加しているグループ　または　個人として「すでに登録した名刺と重複しているか」を確認したあと、
入力フォームがでますので適宜コピー&ペーストまたは修正の後、登録してください。
```
(なにも入力していなくてもデフォルトで入っている値は、GoogleCloudNaturalLanguageAPIの意図解釈結果による値です。必要に応じて編集可能です。)
```
...
+ VisionAPIで認識した文字を、GoogleCloudPlatformのNaturalLanguageAPIで意図解釈し、名刺データ新規作成用フォームに自動入力
